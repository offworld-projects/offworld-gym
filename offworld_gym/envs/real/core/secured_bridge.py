...
class SecuredBridge(metaclass=Singleton):
    ...
    def get_last_heartbeat(self):
        ...
        api_endpoint = "http://{}:{}/{}".format(self.server_ip, self.secured_port, HeartBeatRequest.URI)
        response = requests.post(url = api_endpoint, json = req.to_dict()) 
    ...
    def perform_action(self, action_type, channel_type):
        ...
        api_endpoint = "http://{}:{}/{}".format(self.server_ip, self.secured_port, ActionRequest.URI)
    ...
    def perform_reset(self, channel_type=Channels.DEPTH_ONLY):
        ...
        api_endpoint = "http://{}:{}/{}".format(self.server_ip, self.secured_port, ResetRequest.URI)
        response = requests.post(url = api_endpoint, json = req.to_dict()) 
        response_json = json.loads(response.text)
