...
from offworld_gym.utils.offworld_gym_utils import ImageUtils
from offworld_gym.utils.offworld_gym_utils import GazeboUtils
from offworld_gym.exception.gym_exception import GymException
...
class ActionType(Enum):
    """Actions supported by the environment
    """
    LEFT = 0
    RIGHT = 1
    FORWARD = 2
...
class RosbotMonolithEnv(GazeboGymEnv):
    """Gym environment with a rosbot and a monolith on an uneven terrain

    A RL agent learns to reach the goal(monolith) in shorted time

    Usage:
        env = gym.make('RosbotMonolithSimEnv-v0', channel_type=ChannelType.DEPTHONLY)
        env = gym.make('RosbotMonolithSimEnv-v0', channel_type=ChannelType.RGB_ONLY)
        env = gym.make('RosbotMonolithSimEnv-v0', channel_type=ChannelType.RGBD)
    """
    _PROXIMITY_THRESHOLD = 0.20

    def __init__(self, channel_type=ChannelType.DEPTH_ONLY, random_init=False):
        super(RosbotMonolithEnv, self).__init__(package_name='gym_rosbot_monolith', launch_file='env_bringup.launch')

        assert isinstance(channel_type, ChannelType), "Channel type is not of ChannelType."
    ...
    def _get_state(self):
        ...
        if self.channel_type == ChannelType.DEPTH_ONLY:
            state = depth_img
        elif self.channel_type == ChannelType.RGB_ONLY:
            state = img
        elif self.channel_type == ChannelType.RGBD:
    ...
    def _calculate_reward(self):
        ...
        if dst < RosbotMonolithEnv._PROXIMITY_THRESHOLD:
            reward = 1.0
            done = True
        else:
            reward = -0.01
    ...
    def step(self, action):
        ...
        assert isinstance(action, ActionType) or isinstance(action, int), "Action type is not recognized."
